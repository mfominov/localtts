...
Ссылки вида `§2.5.1` перед озвучкой превращаются в `в разделе 2 точка 5 точка 1`, а на входе в TTS цифры читаются словами (`два точка пять точка один`), чтобы Silero не глотал номера. В плеере снова показывается `§2.5.1`.

Латинские бренды/аббревиатуры (`PSLC`, `ROI`, …) задаются в `patterns/default.yml` в секции `pronounce:` и подставляются только перед TTS.
